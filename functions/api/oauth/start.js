...
export async function onRequestGet({ env, request }) {
  ...
  authorizeUrl.searchParams.set('clientId', env.IMWEB_CLIENT_ID || DEFAULT_CLIENT_ID);
  authorizeUrl.searchParams.set('redirectUri', redirectUri);
  authorizeUrl.searchParams.set('scope', REQUIRED_SCOPE);
  authorizeUrl.searchParams.set('state', crypto.randomUUID());
  authorizeUrl.searchParams.set('siteCode', env.IMWEB_SITE_CODE || DEFAULT_SITE_CODE);
